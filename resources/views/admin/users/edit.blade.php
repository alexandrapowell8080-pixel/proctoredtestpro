...
@section('seo_title', 'Edit Admin')
@section('content')
<div class="max-w-3xl mx-auto space-y-6">
    <header class="flex items-center justify-between border-b border-[hsl(var(--border))] pb-5">
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Edit Administrator</h1>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-6">
            ...
            <div class="pt-4 border-t border-gray-200 flex justify-end">
                <button type="submit"
                    class="bg-blue-600 text-white font-bold py-2.5 px-6 rounded-lg hover:bg-blue-700 transition-colors shadow-sm">Update
                    User</button>
            </div>
        </form>
    </div>
